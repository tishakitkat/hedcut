...
void insertionSort(std::vector< std::pair<float, cv::Point>> point){
    //std::cout<<"insertion sort"<<std::endl;
     ...
     //contant time (just instructions)
...
void CVT::vor(cv::Mat &  img)
{
    ...
    /*
    if(useqsort){
    file = fopen("compareSort.txt", "a+" );
    }
    else{
        file = fopen("compareNormal.txt", "a+" );
    }
    */
    //average = average + ((double)endTime-startTime)/ (CLOCKS_PER_SEC/1000);
    //counter++;
    //fprintf(file,"total sort time is %f\n",((double)endTime-startTime)/ (CLOCKS_PER_SEC/1000));
    //fclose(file);
...
void CVT::compute_weighted_cvt(cv::Mat &  img, std::vector<cv::Point2d> & sites)
{
	...
	do
	{
        if(timeout)
            ...
            //std::cout<<"it timed out at "<<timeout<<" iterations "<<std::endl;
                break;
    ...
    fprintf(file,"Average time is %f\n",average);
    fclose(file);
    */
    //if (debug) cv::waitKey();
