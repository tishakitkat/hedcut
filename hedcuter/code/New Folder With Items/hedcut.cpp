...
bool Hedcut::build(cv::Mat & input_image, int n)
{
	...
	//initialize cvt
	//so it does use the ctv centroidal veronoi diagram
	CVT cvt;
...
{
	//check if configuration for the graph that removes clusters is true
	UnclusterGraph g;
	if(this->cluster_value > 0 || this->cluster_dist > 0.0){
		this->usegraph = true;
		g.usegraph = true;
	...
	int count = 0;
	//random number generator for generation from a specific distribution
	...
	while (count < n)
	{
		...
		//cout<<"the gussian is "<<gr<<" and the value is "<<value<<" and image at is "<<img.at<uchar>(r, c)<<"\n";
