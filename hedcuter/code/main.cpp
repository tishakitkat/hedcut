...
int main(int argc, char ** argv)
{
	...
	if (argc < 2)
	{
		cout << " Usage: " << argv[0] << " [-n #_of_disks -radius disk_radius -uniform_radius -iteration #_of_CVT_iterations -maxD max_CVF_site_displacement -gclusterv number_of_disk_in_cluster -gclusterdist minimum_distance_between_disks -useqsorting -timeout_count max_number_of_iterations_to_timeout] image_file_name" << endl;
		return -1;
	}
	...
	for (int i = 1; i < argc; i++)
	{
		if (argv[i][0] == '-')
		{
			...
			else if (string(argv[i]) == "-gclusterv" && i + 1 < argc) hedcut.cluster_value = atoi(argv[++i]);
			else if (string(argv[i]) == "-gclusterdist" && i + 1 < argc) hedcut.cluster_dist = atof(argv[++i]);
			else if (string(argv[i]) == "-useqsorting" && i + 1 < argc) hedcut.useqsort = true;
			else if (string(argv[i]) == "-timeout_count" && i + 1 < argc) hedcut.timeout =atoi(argv[++i]);
			else
